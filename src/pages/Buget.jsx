...
export default function Buget({ session }) {
  ...
  async function save() {
    ...
    if (editEntry) await supabase.from('budget_entries').update(payload).eq('id', editEntry.id)
    else           await supabase.from('budget_entries').insert(payload)
    setShowModal(false)
    load()
  }
    ...
    <div className="page fade-in">
      ...
      {tableError && (
        <div className="card text-center py-10 space-y-3">
          ...
          <p className="text-white font-semibold">Tabelul nu există încă</p>
          <p className="text-slate-400 text-sm">
            Rulează <code className="text-brand-orange bg-dark-700 px-1.5 py-0.5 rounded">MIGRATION_buget.sql</code> în Supabase SQL Editor, apoi reîncarcă pagina.
          </p>
        </div>
      )}
      ...
      {!tableError && (
        <>
          ...
          {(tab === 'luna' || tab === 'statistici') && (
            <div className="flex items-center justify-between mb-3">
              <button onClick={prevMonth}
                className="w-9 h-9 flex items-center justify-center rounded-xl bg-dark-700 text-slate-300 hover:bg-dark-600 text-lg">
                ‹
              </button>
              <p className="text-sm font-semibold text-white">{MONTHS_FULL[viewMonth]} {viewYear}</p>
              <button onClick={nextMonth} disabled={isCurrentMonth}
                className="w-9 h-9 flex items-center justify-center rounded-xl bg-dark-700 text-slate-300 hover:bg-dark-600 disabled:opacity-30 text-lg">
                ›
              </button>
            </div>
          )}
          ...
          {tab !== 'statistici' && (
            <div className="card mb-4">
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  <p className="text-base font-bold text-brand-green">{fmt(totalIncome)}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">↑ Venituri</p>
                </div>
                <div>
                  <p className={`text-base font-bold ${balance >= 0 ? 'text-white' : 'text-red-400'}`}>{fmt(balance)}</p>
              ...
              <div className="card">
                ...
                <div className="flex gap-4 mt-2">
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-sm bg-brand-green/50" />
                    <span className="text-[10px] text-slate-500">Venituri</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-sm bg-red-400/50" />
                    <span className="text-[10px] text-slate-500">Cheltuieli</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {tab !== 'statistici' && (
            loading ? (
              <p className="text-center text-slate-500 text-sm py-8">Se încarcă...</p>
            ) : displayList.length === 0 ? (
              <div className="card text-center py-10">
                <p className="text-3xl mb-2">💸</p>
                <p className="text-slate-400 text-sm">
                  {tab === 'luna' ? 'Nicio înregistrare luna asta.' : 'Nicio înregistrare încă.'}
                </p>
              </div>
            ) : (
                  ...
                  <div key={e.id} className="card flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500">
                        {new Date(e.date + 'T12:00:00').toLocaleDateString('ro-RO', {
                          day: 'numeric', month: 'short',
                          year: tab === 'toate' ? 'numeric' : undefined
                        })}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-1 shrink-0">
                      <button onClick={() => openEdit(e)}
                        className="text-xs bg-dark-700 text-slate-400 px-2 py-1 rounded-lg hover:bg-dark-600">✏️</button>
                      <button onClick={() => del(e.id)}
                        className="text-xs bg-red-500/10 text-red-400 px-2 py-1 rounded-lg hover:bg-red-500/20">🗑</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input className="input text-lg font-bold" type="number" step="0.01" placeholder="0.00"
              autoFocus value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} />
          </div>
